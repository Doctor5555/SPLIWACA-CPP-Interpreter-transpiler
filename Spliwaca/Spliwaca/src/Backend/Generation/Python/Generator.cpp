...
namespace Spliwaca
{
	...
	void PythonGenerator::GenerateStatements(std::shared_ptr<Statements> statements)
	{
		...
		{
			SN_PROFILE_SCOPE("Statement loop - PythonGenerator::GenerateStatements(std::shared_ptr<Statements>)");
	...
	std::string PythonGenerator::ParseRaw(std::shared_ptr<Token> token)
	{
		...
		for (char c : contents)
		{
			SN_PROFILE_SCOPE("Token contents loop - PythonGenerator::ParseRaw(std::shared_ptr<Token>)");
			if (inIdent)
			{
				SN_PROFILE_SCOPE("inIdent - PythonGenerator::ParseRaw(std::shared_ptr<Token>)");
				if (c == ' ')
				{
